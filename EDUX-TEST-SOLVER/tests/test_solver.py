...
def test_bruteforce(page: Page) -> None:
    ...
    while True:
        print("[INFO] When ready to start, press Enter to click 'Lam bai tap' and capture payload. Type 'q' and Enter to quit.\n")
        user_input = input()
        ...
        try:
            ...
            print(f"[WARN] Failed to start test or capture response (Are you on the correct page?): {e}")
            continue
        ...
        if not answers:
            print("[WARN] No answers parsed from dialog input. Stopping this attempt.")
            continue
